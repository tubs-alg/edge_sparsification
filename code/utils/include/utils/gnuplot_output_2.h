...
//
// $URL: https://github.com/CGAL/cgal/blob/releases/CGAL-5.0.3/Cone_spanners_2/include/CGAL/gnuplot_output_2.h $
...
void gnuplot_output_2 (const Graph& g, const std::string& prefix, std::vector<typename Graph::edge_descriptor> *vec = nullptr, std::optional<bool> labels = false, std::vector<std::pair<std::pair<double,double>,std::pair<double,double>>> *cut = nullptr);
...
*/
template <typename Graph>
std::string gnuplot_edge_list (const Graph& g, std::vector<typename Graph::edge_descriptor> *vec);
    ...
    for (boost::tie(eit, ee) = boost::edges(g); eit != ee; ++eit) {
        ...
        if (vec == nullptr) {
            ...
            for (auto e = vec->begin(); e != vec->end(); ++e) {
                if (boost::source(*e, g) == boost::source(*eit, g) && boost::target(*e, g) == boost::target(*eit, g)) {
                    ss << " lt rgb \"red\"";
                    break;
                }
            }
...
std::string gnuplot_vertex_list(const Graph& g) {
    ...
    for (boost::tie(vit, ve) = boost::vertices(g); vit != ve; ++vit) {
        ss << to_double(g[*vit].x()) << "  " << to_double(g[*vit].y()) << "  " << *vit << std::endl;
    }
    return ss.str();
}

template <typename Graph>
void gnuplot_output_2 (const Graph& g, const std::string& prefix, std::vector<typename Graph::edge_descriptor> *vec, std::optional<bool> labels, std::vector<std::pair<std::pair<double,double>,std::pair<double,double>>> *cut)
{
    ...
    */
    ...
    fs << Gnuplot_edges_2<Graph>::gnuplot_edge_script(g, vec);
    fs << std::endl;
    if (cut != nullptr) {
        for (auto e = cut->begin(); e != cut->end(); ++e) {
            fs << "set arrow from ";
            fs << e->first.first << "," << e->first.second;
            fs << " to ";
            fs << e->second.first << "," << e->second.second;
            fs << " as 1";
            fs << " lt rgb \"red\"";
            fs << std::endl;
        }
    ...
    // plot the vertices
    fs << "plot \"" << boost::filesystem::path{prefix}.filename().string() << ".v\" with points pt 7 ps 0.8 lt rgb \"blue\"";
    if (labels == true) {
        fs << ",\\" << std::endl;
        fs << "\"" << boost::filesystem::path{prefix}.filename().string() << ".v\" using 1:2:($0) with labels tc rgb \"black\" offset (0,0) font \"Arial Bold\" notitle ";
    }
...
struct Gnuplot_edges_2<Graph, boost::undirectedS> {
    ...
    static std::string gnuplot_edge_script(const Graph& g, std::vector<typename Graph::edge_descriptor> *vec)
    {
        std::stringstream ss;
        ss << "set style arrow 1 nohead lc rgb \"black\"" << std::endl;
        ss << std::endl;
        ss << "# edges" << std::endl;
        ss << gnuplot_edge_list(g, vec);
        ss << "# end of edges" << std::endl;
